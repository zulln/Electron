...
var self = this;
self.AnswerBlockTypesEnum = AnswerBlockTypesEnum, self.onBlockChange = function() {
...
var self = this;
self.keyboardShortcutLabels = parseKeyboardShortcutsConfigs(getKeyboardShortcutLabel, KEYBOARD_SHORTCUTS[SHORTCUTS_PLATFORM]), 
self.wordCountClassNames = function() {
var classNames = [];
...
classNames;
}, self.addContentFocus = function() {
$scope.$element.addClass(CLASSNAME_FOCUS_MOD);
}, self.removeContentFocus = function() {
$scope.$element.removeClass(CLASSNAME_FOCUS_MOD);
}, self.getWordCountDisplay = function(wordCount, question) {
...
var CMD_CHAR = "\u2318";
return function(config, forcedPlatform) {
var label = "", mods = config.modifiersAND || [], platform = forcedPlatform || detectPlatform();
return mods.indexOf("meta") > -1 && ("mac" === platform ? (label += CMD_CHAR, mods.length > 1 && (label += "+")) :label += "Meta+"), 
mods.indexOf("ctrl") > -1 && (label += "Ctrl+"), mods.indexOf("shift") > -1 && (label += "Shift+"), 
mods.indexOf("alt") > -1 && (label += "mac" === platform ? "Option+" :"Alt+"), label += config.keysOR[0].toUpperCase();
};
}
...
var useCached = "undefined" != typeof useCachedResult ? useCachedResult :!0;
if (useCached && cached) return result;
...
"use strict";
function getVendorPrefixedCssPropertyService($window) {
var testEl, cached = {}, result = {}, prefixes = [ "Moz", "Webkit", "O", "ms" ];
return function(property, useCachedResult) {
var useCached = "undefined" != typeof useCachedResult ? useCachedResult :!0;
if (useCached && cached[property]) return result[property];
if (testEl || (testEl = $window.document.createElement("div")), property in testEl.style) return cached[property] = !0, 
result[property] = property, property;
for (var upper = property.charAt(0).toUpperCase() + property.slice(1), len = prefixes.length; len; len--) if (prefixes[len] + upper in testEl.style) return cached[property] = !0, 
...
getVendorPrefixedCssPropertyService.$inject = [ "$window" ];
...
onScrollTopItems(viewportTop), onScrollBottomItems(viewportTop + scrollEl[0].offsetHeight);
}
function onScrollTopItems(viewportTop) {
var position = "top", managerTop = manager.top;
if (0 === viewportTop ? setCurrentItem(null, position) :managerTop.next && managerTop.next.top - viewportTop <= 0 ? setCurrentItem(managerTop.next, position) :managerTop.current && viewportTop < managerTop.current.top && setCurrentItem(managerTop.prev, position), 
managerTop.current) {
var dockedBottomHeight = managerTop.current.group.docked.bottom ? managerTop.current.group.docked.bottom.height :0, groupBottomOverflow = managerTop.current.group.bottom - (viewportTop + managerTop.current.height + dockedBottomHeight), nextOverflow = managerTop.next ? managerTop.next.top - managerTop.next.height - viewportTop :0;
...
if (viewportBottom === scrollEl[0].scrollHeight ? setCurrentItem(null, position) :managerBottom.next && managerBottom.next.bottom - viewportBottom > 0 ? setCurrentItem(managerBottom.next, position) :managerBottom.current && viewportBottom > managerBottom.current.bottom && setCurrentItem(managerBottom.prev, position), 
managerBottom.current) {
var dockedTopHeight = managerBottom.current.group.docked.top ? managerBottom.current.group.docked.top.height :0, groupTopOverflow = managerBottom.current.group.top + managerBottom.current.height + dockedTopHeight - viewportBottom, nextOverflow = managerBottom.next ? managerBottom.next.bottom + managerBottom.current.height - viewportBottom :0;
...
var value = null === amount || "undefined" == typeof amount ? "" :"translate3d(0," + amount + "px,0)";
...
return $delegate.throttle = function(callback) {
var alreadyQueued;
return function() {
var self = this, queueArgs = arguments;
alreadyQueued || (alreadyQueued = !0, $delegate(function() {
callback.apply(self, queueArgs), alreadyQueued = !1;
}));
};
}, $delegate;
}
