	...
	function stickyManagerService(
		...
		function setupManager(scrollCtrl) {
			...
			function add($element, $clone, groupCtrl, docked) {
				...
				var group = groups.filter(function(groupToFilter) {
			...
			function refreshElementsPosition(position) {
				...
				items = manager[position].items = items.sort(function(a, b) {
					if (position === "top") {
			...
			function onScroll() {
				var fold = scrollEl.prop("scrollTop");
				onScrollTopItems(fold);
				onScrollBottomItems(fold);
			}
			...
			function onScrollTopItems(fold) {
				...
				} else if (managerTop.next && managerTop.next.top - fold <= 0) {
					setCurrentItem(managerTop.next, position);
				// Sticky the previous item if we've scrolled up past
				// the original position of the currently stickied item.
				} else if (managerTop.current && fold < managerTop.current.top) {
				...
				if (managerTop.current) {
					...
					var groupBottomOverflow = managerTop.current.group.bottom - (fold + managerTop.current.height + dockedBottomHeight);
					var nextOverflow = managerTop.next ? managerTop.next.top - managerTop.next.height - fold : 0;
			...
			function onScrollBottomItems(fold) {
				...
				fold += scrollEl[0].offsetHeight;

				// At the bottom?
				if (fold === scrollEl[0].scrollHeight) {
					setCurrentItem(null, position);
				// Sticky the next item if we've scrolled past its position.
				} else if (managerBottom.next && managerBottom.next.bottom - fold > 0) {
					setCurrentItem(managerBottom.next, position);
				// Sticky the previous item if we've scrolled down past
				// the original position of the currently stickied item.
				} else if (managerBottom.current && fold > managerBottom.current.bottom) {
				...
				if (managerBottom.current) {
					...
					var groupTopOverflow = managerBottom.current.group.top + managerBottom.current.height + dockedTopHeight - fold;
					var nextOverflow = managerBottom.next ? managerBottom.next.bottom + managerBottom.current.height - fold : 0;
			...
			function translate(item, amount) {
				...
				var value = amount === null || amount === undefined ? "" : "translate3d(0," + amount + "px,0)";
